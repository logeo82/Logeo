...
def market_search():
    ...
    try:
        args=logeo.request.args; p={k:args.get(k) for k in ('q','type','transaction','ville','cp','dept','region','prix_min','prix_max','surface_min','surface_max','pieces_min','chambres_min','dpe','ges','sort')}
        p['page_size']=min(int(args.get('page_size','10') or 10),25)
        data=_api('/annonces',p); items=data.get('items',[]) if isinstance(data,dict) else []
...
def market_detail(source,reference):
    ...
    if not u or u['role']!='owner': return jsonify(error='Connexion propriétaire requise'),403
    try:
        data=_detail(source,reference); annonce=data.get('annonce',data) if isinstance(data,dict) else data
        if not isinstance(annonce,dict): return jsonify(error='Réponse détail invalide'),502
        return jsonify(ok=True,annonce=annonce)
    except Exception as e:return jsonify(error=f'Détail indisponible : {e}'),502
