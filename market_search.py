...
def market_search():
    ...
    try:
        ...
        data=_api('/annonces',p);return jsonify(ok=True,total=data.get('total',0),items=data.get('items',[]),next_cursor=data.get('next_cursor'))
...
def owner_market_page():
    ...
    return Response('''<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>LOGEO — Recherche multi-portails</title><style>body{font-family:system-ui;background:#f4f6fa;color:#172033;margin:0}main{max-width:1000px;margin:auto;padding:18px}.card{background:#fff;padding:18px;border-radius:16px;margin:12px 0;box-shadow:0 4px 18px #0001}input,select{padding:11px;border:1px solid #d6dbe5;border-radius:10px;font:inherit;width:100%;box-sizing:border-box}button{padding:11px 15px;border:0;border-radius:10px;background:#111827;color:white;font-weight:700;margin-top:8px;cursor:pointer}.grid{display:grid;grid-template-columns:2fr 1fr 1fr;gap:10px}@media(max-width:650px){.grid{grid-template-columns:1fr}.grid .wide{grid-column:auto}}.muted{color:#667085}.err{color:#b42318}.ok{color:#087443}.item{display:flex;gap:14px;border-top:1px solid #eee;padding:14px 0}.item img{width:150px;height:110px;object-fit:cover;border-radius:10px;background:#eef2f6}@media(max-width:650px){.item{flex-direction:column}.item img{width:100%;height:180px}}</style></head><body><main><div class="card"><a href="/" style="text-decoration:none">← Retour à LOGEO</a><h1>🌐 Recherche multi-portails</h1><p class="muted">Recherche d'annonces agrégées depuis plusieurs portails et réseaux immobiliers.</p><div class="grid"><label class="wide">Ville<input id="ville" value="Montauban" placeholder="Montauban"></label><label>Type<select id="type"><option value="">Tous</option><option value="Appartement">Appartement</option><option value="Maison">Maison</option><option value="Terrain">Terrain</option></select></label><label>Transaction<select id="tx"><option value="">Toutes</option><option value="vente">Vente</option><option value="location">Location</option></select></label></div><button id="go">🔎 Rechercher</button><p id="msg" class="muted"></p></div><div class="card"><div id="results"></div></div></main><script>const esc=v=>String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));document.getElementById('go').onclick=async()=>{const b=document.getElementById('go'),m=document.getElementById('msg'),r=document.getElementById('results');b.disabled=true;m.textContent='Recherche…';r.innerHTML='';try{const q=new URLSearchParams({ville:document.getElementById('ville').value.trim(),page_size:'20'});if(document.getElementById('type').value)q.set('type',document.getElementById('type').value);if(document.getElementById('tx').value)q.set('transaction',document.getElementById('tx').value);const x=await fetch('/api/market-search?'+q);const d=await x.json();if(!x.ok)throw Error(d.error||'Recherche impossible');m.textContent=(d.total??d.items.length)+' annonce(s) trouvée(s).';r.innerHTML=(d.items||[]).map(a=>{const img=Array.isArray(a.images)&&a.images.length?a.images[0]:'';return '<div class="item">'+(img?'<img src="'+esc(img)+'">':'<div style="width:150px;height:110px;border-radius:10px;background:#eef2f6"></div>')+'<div><h2 style="margin:0 0 6px">'+esc(a.title||'Annonce')+'</h2><b>'+esc(a.price)+' €</b> · '+esc(a.surface)+' m² · '+esc(a.rooms||'')+' pièce(s)<br><span class="muted">📍 '+esc(a.city||'')+' · 🏢 '+esc(a.real_estate_network||a.seller_name||a.source||'')+' · 📷 '+esc(a.images_count||0)+' photo(s)</span><p>'+esc(a.description||'')+'</p></div></div>'}).join('')||'<p class="muted">Aucune annonce trouvée.</p>'}catch(e){m.textContent='❌ '+e.message;m.className='err'}finally{b.disabled=false}};</script></body></html>''',mimetype='text/html')
