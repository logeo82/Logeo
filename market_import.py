...
def market_import():
    ...
    try:
        ...
        if str(item.get('source') or '') != source:
            # The reference is the stable identifier; keep the returned record if the source differs.
        ...
        try:
            try:
                c.execute("ALTER TABLE listings ADD COLUMN photos TEXT DEFAULT '[]'")
            except Exception:
                pass
            existing = None
            if external:
                existing = c.execute('SELECT * FROM listings WHERE source_url=?', (external,)).fetchone()
            if existing:
        ...
        payload = {
            ...
            'parking_interior': item.get('parking_interior'),
            'parking_exterior': item.get('parking_exterior'),
...
def market_import_ui(response):
    try:
        if response.content_type and response.content_type.startswith('text/html'):
            ...
            if '/owner/market' in (request.path or '') and 'marketImportScript' not in page:
                script = '''<script id="marketImportScript">(function(){let selected=null;function addButton(){if(!selected)return;const d=document.getElementById('detail');if(!d||!d.querySelector('.card'))return;if(document.getElementById('marketImportBtn'))return;const b=document.createElement('button');b.id='marketImportBtn';b.type='button';b.textContent='📥 Importer cette annonce dans LOGEO';b.style.cssText='width:100%;font-size:16px;margin-top:14px';b.onclick=async()=>{b.disabled=true;b.textContent='⏳ Importation…';try{const r=await fetch('/api/market-import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:selected.source,reference:selected.reference})});const x=await r.json();if(!r.ok)throw Error(x.error||'Import impossible');b.textContent=x.duplicate?'⚠️ Déjà importée dans LOGEO':'✅ Annonce importée dans LOGEO';}catch(e){b.disabled=false;b.textContent='❌ '+e.message;}};d.querySelector('.card').appendChild(b)}function watch(){const r=document.getElementById('results');if(!r||r.dataset.marketWatch)return;r.dataset.marketWatch='1';r.addEventListener('click',e=>{const item=e.target.closest('.item');if(!item)return;const i=Number(item.dataset.i);setTimeout(()=>{try{const items=window.__marketItems||[];selected=items[i]||null;addButton()}catch(_){}},50)});new MutationObserver(addButton).observe(document.getElementById('detail')||document.body,{childList:true,subtree:true})}setInterval(watch,300);})();</script>'''
