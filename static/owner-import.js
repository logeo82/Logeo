...
function normalizeListing(l){
 const x=Object.assign({score:100,reasons:[],photos:[]},l||{});
 if(typeof x.photos==='string'){try{x.photos=JSON.parse(x.photos||'[]')}catch(e){x.photos=[]}}
 if(!Array.isArray(x.photos))x.photos=[];
 return x;
}
function loadOwnerDetailScript(){
 if(window.openListingDetail)return Promise.resolve();
 return new Promise(function(resolve,reject){const s=document.createElement('script');s.src='/static/detail.js?v=owner-detail-2';s.onload=resolve;s.onerror=reject;document.head.appendChild(s)});
}
async function wireOwnerListings(){
 ...
 try{
  ...
  const listings=(Array.isArray(x.listings)?x.listings:[]).map(normalizeListing);
  if(!listings.length){box.dataset.detailWired='1';return}
  await loadOwnerDetailScript();
  window.allMatches=listings;
  box.innerHTML='<div class="card"><h2>📢 Mes annonces</h2>'+listings.map(function(l){return '<button type="button" class="secondary logeo-owner-listing" data-owner-listing="'+String(l.id).replace(/"/g,'&quot;')+'" style="display:block;width:100%;text-align:left;margin:10px 0;padding:15px">🏠 <b>'+escapeHtml(l.title||'Annonce')+'</b><br><span class="muted">'+escapeHtml(l.price||'')+' € — '+escapeHtml(l.city||'')+'</span><br><small>👉 Ouvrir la fiche LOGEO</small></button>'}).join('')+'</div>';
  box.querySelectorAll('[data-owner-listing]').forEach(function(b){b.onclick=function(){window.allMatches=listings;window.openListingDetail(Number(b.dataset.ownerListing))}});
  box.dataset.detailWired='1';
 }catch(e){console.error('LOGEO owner listing bridge',e)}
}
