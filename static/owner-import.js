...
function addOwnerImporter(){
 ...
 document.getElementById('ownerImportBtn').onclick=async function(){
  ...
  try{const r=await fetch('/api/import-url',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url})});const x=await r.json();if(!r.ok)throw Error(x.error||'Import impossible');msg.textContent=x.duplicate?'⚠️ Cette annonce est déjà dans LOGEO.':'✅ Annonce importée dans LOGEO !';msg.className='ok';document.getElementById('ownerImportUrl').value='';if(typeof loadOwner==='function')loadOwner();}
...
function boot(){
 addOwnerImporter();
 const obs=new MutationObserver(function(){if(addOwnerImporter()) obs.disconnect();});
 obs.observe(document.body,{childList:true,subtree:true});
 setTimeout(function(){obs.disconnect();addOwnerImporter();},30000);
}
