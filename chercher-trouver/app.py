...
def number(v):
    ...
    a=m.group(1).replace(" ","").replace(".",""); d=m.group(2)
    try: return float(a+("."+d if d else ""))
...
def extract(url, raw):
    ...
    price=number(first(offers.get("price"),obj.get("price"),p.meta.get("product:price:amount"),p.meta.get("price")))
    surface=number(first(obj.get("surface"),obj.get("area"),p.meta.get("surface")))
    ...
    if price is None:
        m=re.search(r"(?:loyer|prix|montant)[^0-9]{0,80}([0-9][0-9 .]{2,})\s*€",visible,re.I) or re.search(r"([0-9][0-9 .]{2,})\s*€\s*(?:/\s*mois|par mois)",visible,re.I)
        if m: price=number(m.group(1))
    if surface is None:
        m=re.search(r"(?:surface|superficie)[^0-9]{0,40}([0-9]+(?:[,.][0-9]+)?)\s*m²",visible,re.I) or re.search(r"([0-9]+(?:[,.][0-9]+)?)\s*m²",visible,re.I)
    ...
    return {"title":str(title or f"{typ} à louer à {city}").strip(),"city":str(city).strip(),"price":price,"surface":surface or 0,"type":typ,"furnished":furnished,"description":str(desc).strip()[:10000],"source":urlparse(url).netloc.lower().replace("www.",""),"source_url":canonical(url)}

def fetch_listing(url):
    p=urlparse(url)
    if p.scheme not in ("http","https"): raise ValueError("URL invalide")
    req=Request(url,headers={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/151 Safari/537.36","Accept-Language":"fr-FR,fr;q=0.9,en;q=0.8"})
    try:
