...
def register():
 ...
 c=db()
 try:
  cur=c.execute('INSERT INTO users(email,password_hash,name,role,created_at) VALUES(?,?,?,?,?)',(email,hp(password),x.get('name',''),role,datetime.utcnow().isoformat()));c.commit();session['uid']=cur.lastrowid
